...
实际查看皮甲女战士及燃翼巨鸟结果图。完整英文prompt含use image for face reference，中文未提该句；属于可自行提供人脸照片的替换模板，未指认必须找回的特定原照片。使用英文版仍需提供自选参考照片；本次不判断人脸一致性。
...
月下古装女子舞蹈直播界面，弹幕与月景均属于结果。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
西安地标与美食的水彩地图。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
“帮这个产品生成宣传图”是可替换产品输入模板；现存是茶π广告成图，使用者需要提供要宣传的产品素材。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
已实际看图并阅读全文。AI眼镜组件爆炸图与卖点；文本直接要求构件示意。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：RAG流程、组件与注释的技术讲解图。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
源文字明确“原文未公开”；归档图文存在，但不能视为完整提示词案例。作为 style\_reference 保留。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
短歌行文字与举杯古人月景，诗词全文要求是输出文本。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
江面小舟、山石和整列古文的长卷；prompt同时提赤壁怀古与赤壁赋，此处仅确认长卷结果资料。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。原图确认七个白底动物科普海报拼接，含眼镜王蛇、甲虫、变色龙、狮、雪豹、裸鼹鼠、远东豹；全部为主题模板输出集合。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：红衣女子、彼岸花与诗词的暗调画面。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
极限性质英文数学教学图，模板主题槽可替换。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
同形香水瓶在春花、海岸、秋叶、冰雪四种场景；产品和材质是替换槽。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。LOUIS VUITTON字样时装杂志封面及女性肖像；完整文字给出人像风格与服装。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：NOIR黑色连帽衫与湿地面街头广告。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
法国新浪潮人物撕纸电影海报，主体和标题为通用替换条件，未指定独立原始照片。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
浅色丝线质感的鸟、花枝立体刺绣。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已复核为可替换输入模板。复用时需自备：角色参考图（image1）；本库未保存原示例的独立输入，不能承诺原样复现示例身份。主会话已复读完整原文，图文要求中没有必须另取某个独有风格/前序作品才能理解的缺口。初审观察：已实际看图并阅读全文。原图为灰度女性16格动作示意，含箭头编号；prompt明确CHARACTER=image1，现有资产只有完成动作表，没有该独立角色输入，不能将格内人物裁切视为已提供image1。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：巨型网球拍上人物与两幅运动肖像三联画。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
足球球员生涯涂鸦海报，文字为球员名模板；如保留用户指定照片需自备，未要求某张已知源图。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
红色运动服女性坐在巨大哑铃旁与STRENGTH字样。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。西楚霸王竖排巨字、暗金人物和红线海报；文字给出暗黑国风版式，无指定输入。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
通用自拍与品牌输入模板；图内用户输入区只是输出版式内容，不能据此认定独立自拍源文件已存。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
