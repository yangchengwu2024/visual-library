...
夜间沙发上戴眼镜女性持手机的近景。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。浅黄底柳岩局部肖像、花形头饰与竖排文字；主题姓名与版式均由文本给定。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：围巾帽子钩织玩偶手提灯笼。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。含可替换主题、人物或参数，样例可采用不同填充值；不把通用占位符视为缺少指定图片。
...
俯拍庭院女子抚橘猫，斑驳光影与文字场景一致。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
拿恐龙玩具的奔跑儿童被涂鸦人物拉住衣服。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。粉色圆角Bichon Shop白色小狗购物袋图标；名称及图标风格由文本给出。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：CAPE TOWN巨幅字母内嵌城市建筑地标。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。含可替换主题、人物或参数，样例可采用不同填充值；不把通用占位符视为缺少指定图片。
...
原文对上传角色的轮廓/护甲/帽衫有具体保持要求，现存仅角色设定输出。可换自己的设计参考复用，但原角色输入尚不能确认。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
Paris 黑色旅行涂鸦配少量蓝色，地点为可替换文本。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
蓝色几何切面与珊瑚色三角元素的短发女性艺术肖像。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
prompt 的 target\_tool 与 target 明示 DALL-E 3 \(ChatGPT\)，属于目标工具声明，不等于实际生成证明；family 保持 unknown，版本不强行填写。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
已实际看图并阅读全文。蓝色橡胶管结构鞋袋与鞋子的棚拍图；Object/brand为参数，品牌形状颜色可由文字指定，无固定附图要求。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：杯内鱼眼视角的草莓碎冰、吸管及饮用女性。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
闭眼女子躺花丛、前景花朵动态模糊，与日文长 prompt 场景一致。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
固定香水瓶外形与原标签需完全保留，使用时必须自供产品原图；现图是布景广告，未见独立瓶图，不能从成品裁切并宣称恢复原始输入。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
海军蓝T恤上的月面宇航员喝饮料与地球图案。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
四层成图顶部呈照片区域；这是结果内嵌区域，不能证明完整原照已归档。风格转译模板可用使用者新照片复用。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
已实际看图并阅读全文。曼哈顿公园草地人群与天际线的水彩插画；全文定义画面，无输入图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：黑场中的分层光面、人物及共享透视标题。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
手持 Rio 票据与立体城市纸雕旅行海报，手和票据为生成场景。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
