...
透明水晶拱框中的意大利地标及镜面倒影。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。六格沙滩微缩女性与柠檬互动、LIMORA饮品；全文详述六场景，sample文字只限制不要抄字，不构成某指定必需输入图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：黑白人像横穿红色光带与FOCUS大字。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。含可替换主题、人物或参数，样例可采用不同填充值；不把通用占位符视为缺少指定图片。
...
樱花路上女子回眸，暖光和飘花场景与文本对应。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
巨大书架夹道、白衣跪者、翻开书与黑烟人形。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。黄紫底墨线男性和犬的半身海报；人物服装动物场景颜色均为可替换文本槽。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：未来巴黎建筑、巨型植物与埃菲尔铁塔艺术海报。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
黑白墨块墨镜侧脸，颈身区 STYLE IS A WEAPON 字样，文字与人物为模板替换项。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
CORN整穗玉米、KERNEL玉米粒与虚线箭头的词汇卡。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
