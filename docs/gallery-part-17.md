...
女子坐巨型紫色手柄与 ENERGY 大字，完整广告输出。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
大脑和电路融合主图及科学模块的蓝紫信息板。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。绿毛衣女生在大学庭院学习与手写涂鸦；文本直接规定全部场景。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：TRANSPARENCY大字与现代玻璃建筑极简海报。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
原文使用 style guide，又完整描述彩色纽扣美学；可以新提供风格参考复用。prompt 同时含 --v 6，实际模型未知；三幅对照图不是已证实的输入/输出配对。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
晨间床边女子与窗光，未要求某张人物原照。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
孙悟空主视觉、法器与人物志十二栏百科海报。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已复核为可替换输入模板。复用时需自备：用户选择的人像照片；本库未保存原示例的独立输入，不能承诺原样复现示例身份。主会话已复读完整原文，图文要求中没有必须另取某个独有风格/前序作品才能理解的缺口。初审观察：已实际看图并阅读全文。单独看原图为中央女性加金牛处女摩羯三个小人拼贴；prompt明确使用provided female subject并零改身份，只有合成输出，没有指定女性独立参考照。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：复古印尼猫薄荷广告、飞起的猫及产品罐。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
PORTO 蓝天城市旅行海报，城市地标为通用文本槽。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
可颂制作从黄油到出炉撕开的八格故事板。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。雪地求生分镜板，包含角色多视图、俯视地图及镜头格；CHARACTER REFERENCE是需生成的板内区块，全文已描述角色，不是要求另附图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：白底墨线粉彩女性半身插画及周围符号。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。含可替换主题、人物或参数，样例可采用不同填充值；不把通用占位符视为缺少指定图片。
...
棒棒糖悬浮霓虹街道的红色广告海报，品牌及文案直接指定。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
粉衣女性彩色主体与四个黑白墨镜肖像拼贴。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。咖啡馆情侣、甜点与涂鸦边框；服装食品颜色为文本占位，无指定照片。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：蓝发女性戴耳机与F1比赛直播信息叠层。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
