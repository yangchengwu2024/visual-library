...
Claude Opus 4\.7 虚构带货界面，男子手持包装、价格卡及评论明确。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
原文要求保留 REFERENCE\_0 中心鸡插画并移动、替换标题；现存图已经是改后汤包标签，未见改前版式原图。 审核只涉及资料输入要求/资产角色，不包含重新生图、身份一致性或像素复现验证。
...
蓝色跑车停在大型客机前的机库照片风画面。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。紫发VTuber抱枕、杯子和徽章周边集合；fan goods通用模板，未指名必须提供某角色原图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：睡衣展示人物与带货聊天、价格卡片。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
MEAN WELL 电源详情页，多视角及应用小图属于成品模块。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
白色运动套装女性持球拍与直播商品卡。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。白衣网球女性直播带货界面；JSON定义商品、模特及缩略图，均为输出UI。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：收音机、随身听、电子表等复古设备图标。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
热带海滨第三人称游戏界面，有地图和任务 HUD，非所需输入截图。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
蓝白渐变球形笑脸拟人角色。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。月下古装男子举杯的抖音直播画面；文案直接指定李白月下直播起舞。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：太空舱人物举牌与直播火箭礼物叠层。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
夜市女子口衔烤串、双手交叉持串，与三刀流场景相符。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
十二张金色盔甲动漫角色卡片排成三行四列。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。李世民玄武门之变朋友圈界面；文案为历史事件UI创作，配图和评论属输出。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：桌面手写中医处方笺。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
以 A 开头物件十乘十网格，是完整字母教学输出图。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
科幻城市、主角与动物伙伴的桃太郎动画主视觉。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。台阶前冰蓝襦裙全身女性；全文给出衣料、光线、居中全身构图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：唐装女子牵引粉色马甲异形的工笔画。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
...
CHRONICLE 浅紫演示封面含产品 UI 示意，prompt 已给定标题文案；网址是背景来源而非指定图片输入。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。
...
苏轼古装人物与小红书帖子排版。已阅读全文并查看样图，图片为结果示例；未发现必须补充某一指定输入图片。
...
已实际看图并阅读全文。暗色车辆中控界面，外有提示气泡；是输出界面的展示截图，不是待修改输入。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
实际查看：手机产品的功能模块化详情页。已通读完整归档指令；图像为该创作方向的结果样例，文本未要求某张特定外部原图。
