...
复核说明：已放大查看公寓正反机位及道具参考板，右上明确标SOURCE REF/REF INPUT并含缩略图。完整prompt将original photo列为输出板内缩略区，现有资产已含该缩略展示，但无法仅据此确认其真实输入来源及独立原文件关系；保留混合展示角色和待核对状态，不武断称缺图。
...
角色依据：公寓正反机位、六个道具细节及右上SOURCE REF输入缩略图；板内同时显示结果和标注的参考输入
