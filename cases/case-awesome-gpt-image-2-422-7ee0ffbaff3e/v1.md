...
复核说明：已实际看图并阅读全文。雪地求生分镜板，包含角色多视图、俯视地图及镜头格；CHARACTER REFERENCE是需生成的板内区块，全文已描述角色，不是要求另附图。 本次核对资料关系、图片角色和输入条件，不代表身份、事实或逐像素生成正确性验收。
...
角色依据：雪地求生分镜板，包含角色多视图、俯视地图及镜头格；CHARACTER REFERENCE是需生成的板内区块，全文已描述角色，不是要求另附图。
