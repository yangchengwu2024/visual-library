...
复核说明：ROGUE VIPER 四区游戏概念板，角色三视和环境参考均是输出内容。已实际看样图并阅读完整 prompt；复核资料关系与输入条件，不代表生成复现或逐项事实准确性验收。

## 检索说明

检索状态：`confirmed`
检索标题：ROGUE VIPER 游戏角色概念设定板
检索描述：四区游戏概念板，包含角色三视、角色表情/装备与环境设定。
检索别名：游戏角色设定板、角色三视、环境概念设定、Rogue Viper
检索关键词：游戏、角色设定、角色三视、概念设定板、环境设定
有效标签：{"categories": ["人物与角色"], "scenes": ["科技", "游戏"], "styles": ["角色设定", "概念艺术", "写实"]}
...
角色依据：ROGUE VIPER 四区游戏概念板，角色三视和环境参考均是输出内容
