...
# Brutalist Freestyle 角色设定表（需要角色参考图）
...
检索状态：`needs_validation`
检索标题：Brutalist Freestyle 角色设定表（需要角色参考图）
检索描述：角色设定输出包含轮廓、护甲、帽衫和表情表，但原始 prompt 依赖上传角色参考图；当前归档没有该输入图，复用时需要替换为自己的角色参考。
检索别名：角色设定表、角色概念图、Brutalist Freestyle
检索关键词：角色设定、概念图、角色参考图、三视/表情表、brutalist
有效标签：{"categories": ["人物与角色"], "scenes": ["科技", "游戏"], "styles": ["角色设定", "概念艺术"]}
